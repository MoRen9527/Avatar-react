...
export function resolveApiBase(mode: string, envBase?: string): string {
  ...
  if (!value) {
    ...
    throw new Error('VITE_API_URL 未设置，请在部署环境中提供 VITE_API_URL 环境变量');
  }
  return value;
}

// 统一获取 API 基址：开发环境未配置则警告并回退，生产环境未配置则直接抛错（fail-fast）
export function getApiBase(): string {
  return resolveApiBase(import.meta.env.MODE, import.meta.env.VITE_API_URL);
}

// 统一常量：在模块加载时解析一次，供全局复用
export const API_BASE_URL: string = getApiBase();

// Auth Server 基址（去掉 /api 前缀）
